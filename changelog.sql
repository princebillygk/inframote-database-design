...
-- changeset princebillygk:5

/* Removes everything */

DROP TABLE reported_usr;
DROP TABLE reported_remote; 
DROP TABLE su;
DROP TABLE issue;
DROP TABLE remote_controller;
DROP TABLE device;
DROP TABLE usr_facebook_auth;
DROP TABLE usr_google_auth;
DROP TABLE usr_email_auth;
DROP TABLE usr;
...
CREATE TABLE IF NOT EXISTS reported_usr (
    ...
    details TEXT NOT NULL,
    is_active BOOL NOT NULL default true,
    FOREIGN KEY (issuer_id) REFERENCES usr(id) ON UPDATE CASCADE ON DELETE NULL,
    FOREIGN KEY (reported_usr_id) REFERENCES usr(id) ON DELETE CASCADE ON UPDATE CASCADE
);
